...
int main() {
    ...
    if (pid > 0) {
        ...
        char buf[SIZE] = {0};
        struct flock lock;
        lock.l_type = F_WRLCK; // Запрещает любому другому процессу читать или записывать в файл, пока блокировка активна.
        lock.l_whence = SEEK_SET; /* Seek from beginning of file.  */
        lock.l_start = 0; /* Offset where the lock begins.  */
        lock.l_len = 0; /* Size of the locked area; zero means until EOF.  */
        while (1) {
            lock.l_type = F_WRLCK;
